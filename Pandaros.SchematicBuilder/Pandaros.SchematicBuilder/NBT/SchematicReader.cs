...
namespace Pandaros.SchematicBuilder.NBT
{
    ...
    public static class SchematicReader
    {
        ...
        public static bool TryGetSchematicMetadata(string name, int colonyId, out SchematicMetadata metadata)
        {
            ...
            {
                metadata = null;
                SchematicBuilderLogger.LogError(ex, "error getting metadata for schematic {0}", name);
            }
        ...
        public static List<FileInfo> GetSchematics(Players.Player player)
        {
            ...
            if (player.ActiveColony != null)
                foreach (var file in Directory.EnumerateFiles(colonySchematics, "*.csschematic"))
                    options.Add(new FileInfo(file));

            foreach (var file in Directory.EnumerateFiles(GameLoader.Schematic_DEFAULT_LOC, "*.csschematic"))
                options.Add(new FileInfo(file));
        ...
        public static void SaveSchematic(Colony colony, Schematic schematic)
        {
            ...
            var fileSave = Path.Combine(colonySaves, schematic.Name + ".csschematic");
            var metaDataSave = Path.Combine(GameLoader.Schematic_SAVE_LOC, schematic.Name + ".csschematic.metadata.json");
            ...
            for (int Y = 0; Y <= schematic.YMax; Y++)
            {
                for (int Z = 0; Z <= schematic.ZMax; Z++)
                {
                    for (int X = 0; X <= schematic.XMax; X++)
                    {
                        ...
                        {
                            var block = ItemId.GetItemId(schematic.Blocks[X, Y, Z].BlockID);

                            if (block != ColonyBuiltIn.ItemTypes.AIR.Id)
                            {
                                var buildType = ItemTypes.GetType(block.Id);
