...
namespace Pandaros.SchematicBuilder.Jobs.Construction
{
    ...
    public class ArchitectBuilder : IConstructionType
    {
        ...
        public void DoJob(IIterationType iterationType, IAreaJob areaJob, ConstructionJobInstance job, ref NPCBase.NPCState state)
        {
            ...
            if (bpi == null)
            {
                SchematicBuilderLogger.Log(ChatColor.yellow, "iterationType must be of type ArchitectIterator for the ArchitectBuilder.");
            ...
            while (true) // move past air
            {
                ...
                if (World.TryGetTypeAt(iterationType.CurrentPosition, out ItemTypes.ItemType foundType))
                {
                    ...
                    {
                        SchematicBuilderLogger.Log(ChatColor.red, $"Index out of range on ArchitectBuilder {adjX}, {adjY}, {adjZ} to a max of {bpi.BuilderSchematic.Blocks.GetLength(0)}, {bpi.BuilderSchematic.Blocks.GetLength(1)}, {bpi.BuilderSchematic.Blocks.GetLength(2)}.");
                    ...
                    if (!foundType.Name.Contains("bedend"))
                        ServerManager.TryChangeBlock(iterationType.CurrentPosition, ItemId.GetItemId(GameLoader.NAMESPACE + ".Selector").Id, new BlockChangeRequestOrigin(job.Owner), ESetBlockFlags.DefaultAudio);
