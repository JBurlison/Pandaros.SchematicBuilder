            ...
            while (true) // move past air
            {
                ...
                if (World.TryGetTypeAt(iterationType.CurrentPosition, out ItemTypes.ItemType foundType))
                {
                    ...
                    if (!foundType.Name.Contains("bedend"))
                        ServerManager.TryChangeBlock(iterationType.CurrentPosition, ItemId.GetItemId(GameLoader.NAMESPACE + ".Selector"), new BlockChangeRequestOrigin(job.Owner), ESetBlockFlags.DefaultAudio);
