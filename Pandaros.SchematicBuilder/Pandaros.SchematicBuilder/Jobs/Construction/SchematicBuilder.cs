...
namespace Pandaros.SchematicBuilder.Jobs.Construction
{
    ...
    public class SchematicBuilder : IConstructionType
    {
        ...
        public void DoJob(IIterationType iterationType, IAreaJob areaJob, ConstructionJobInstance job, ref NPCBase.NPCState state)
        {
            ...
            if (bpi == null)
            {
                SchematicBuilderLogger.Log(ChatColor.yellow, "iterationType must be of type SchematicIterator for the SchematicBuilder.");
            ...
            while (true) // This is to move past air.
            {
                ...
                var mapped = block.BlockID;
                var buildType = ItemTypes.GetType(mapped);
            ...
            else
            {
                ...
                state.SetIndicator(new Shared.IndicatorState(5f, ColonyBuiltIn.ItemTypes.ERRORIDLE.Name));
                AreaJobTracker.RemoveJob(areaJob);
                SchematicBuilderLogger.Log(ChatColor.yellow, "Failed to MoveNext after while. Iterator position: {0}.", iterationType.CurrentPosition);
                return;
            }
        }
